...
func GetTransactionHandle(w http.ResponseWriter, r *http.Request){
	...
	transactionController := database.GetTransactionCollection()

	cursor, err := transactionController.Find(context.TODO(), bson.M{})

	if err != nil{
		http.Error(w, "Error in finding transactions", http.StatusInternalServerError)
		return
	}
	...
	if err!=nil{
		http.Error(w, "Error in marshalling transactions", http.StatusInternalServerError)
		return
	}
...
func GetTransactionByIDHandler(w http.ResponseWriter, r *http.Request){
	...
	filter := bson.M{"transactionId": transID}

	var transaction models.Transaction

	err := transactionController.FindOne(context.Background(), filter).Decode(&transaction)
	if err != nil{
		if err == mongo.ErrNoDocuments{
